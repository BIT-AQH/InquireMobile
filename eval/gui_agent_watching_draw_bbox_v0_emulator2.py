...
def capture_and_pull_screenshot(mobile_image_path, pc_image_path, device_serial='emulator-5554'):
    try:
        ...
        # check_file_cmd = f'adb -s {device_serial} shell "ls {mobile_image_path}"'
        ...
        # print(f'capture_cmd finished!!! for {mobile_image_path}')
        # max_retries = 5
        # # 拉取截图
        ...
        #         if result.returncode == 0:  # 文件存在
        #             log_message(f"Screenshot file exists on device: {mobile_image_path}")
        #             subprocess.run(pull_cmd, shell=True, check=True)
        #             break
        #         else:
        #             log_message(f"Attempt {attempt + 1}: Screenshot file not found on device")
        #             if attempt < max_retries - 1:
        #                 time.sleep(5)  # 等待一秒后重试
        #                 subprocess.run(capture_cmd, shell=True, check=True)  # 重新截图
        #     except Exception as e:
        #         log_message(f"Capture_And_Pull Attempt {attempt + 1} failed with error: {e}")
        subprocess.run(capture_cmd, shell=True, check=True)
        subprocess.run(pull_cmd, shell=True, check=True)
...
def execute_mobile_action(action_json, device_serial='emulator-5554'):
    ...
    try:
        ...
        if action == 'key':
            ...
            cmd = f'{base_cmd} shell input tap {int(x*scatter)} {int(y*scatter)}'
...
def watch_and_execute_oss_action(
    ...
    if not os.path.exists(pc_image_path):
        ...
        pc_image_path = '/Users/aiqihang/code/GUIAgent/images/homescreen/homescreen.png'
    ...
    while True:
        try:
            ...
            if current_file_size != last_file_size:
                ...
                result = execute_mobile_action(latest_action, device_serial)
                ...
                if not os.path.exists(pc_image_path):
                    # temp+=1
                    print(f'{pc_image_path} does not exits, use local homescreen')
                    pc_image_path = '/Users/aiqihang/code/GUIAgent/images/homescreen/homescreen.png'
                local2oss(pc_image_path, oss_image_path)
    ...
    try:
        ...
        for sample_id in range(0, sample_num):
            try:
                ...
                pc_image_folder = os.path.join('/Users/aiqihang/code/GUIAgent/images/emulator', task_name, device, f'step{training_step}', f'sample{sample_id}/')
                pc_image_bbox_folder = os.path.join('/Users/aiqihang/code/GUIAgent/images/emulator_bbox', task_name, device, f'step{training_step}', f'sample{sample_id}/')
...
if __name__ == '__main__':
    ...
    # eval
    cuda_devices = ["device1"] # ,"device2"
    max_steps = 15
    task_name = "eval_inquire_GUI_R1_simulator_en" # eval_mobile_r1_complex_inst
    sample_num = 1
