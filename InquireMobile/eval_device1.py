    ...
    while step < max_steps -1 :
        ...
        if isinstance(action, dict) and "action" in action:
            ...
            if action['action'] == 'call_user':
                ...
                history += f"经过与用户交互，得到用户反馈，{intension}，确认执行，接下来无需调用call_user，请直接执行任务。"
...
def main(args):
    ...
    try:
        for i, instruction in enumerate(tqdm(undone_eval_instructions, desc="Evaluating Instructions")):
            ...
            oss_image_folder_all = f'/data/oss_bucket_0/qihang/gui_agent/images/{task_name}/device1/instruction_{i+1+done_index}/sample_0'
            oss_text_folder_all = f'/data/oss_bucket_0/qihang/gui_agent/instructions/{task_name}/device1/instruction_{i+1+done_index}/sample_0'
