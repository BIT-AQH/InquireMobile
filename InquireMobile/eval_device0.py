...
# 1. 辅助函数
# ==============================================================================
def check_and_read_image(file_path, back_file_path, sleep_duration=1):
    ...
    try:
        ...
        print(f"无法处理图像: {e}")
        # image = Image.open('/data/oss_bucket_0/qihang/gui_agent/images/eval/huawei/step_1/sample_0/step_0.png')
...
def cuda_gc():
    """ 强制 Python 垃圾回收 + 释放 PyTorch CUDA 缓存 / IPC 句柄 """
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()   # 归还未使用显存
        torch.cuda.ipc_collect()   # 清理 CUDA IPC 共享句柄
    ...
    while step < max_steps -1 :
        ...
        if isinstance(action, dict) and "action" in action:
            ...
            if action['action'] == 'call_user':
                ...
                # 希望agent尽量少调用call_user
                history += "经过与用户交互，得到用户反馈，确认执行，接下来无需调用call_user，请直接执行任务。"
...
def main(args):
    ...
    if os.path.exists(args.eval_result_path):
        with open(args.eval_result_path, 'r', encoding='utf-8') as f:
            done_eval_result = [json.loads(line.strip()) for line in f]
    ...
    try:
        for i, instruction in enumerate(tqdm(undone_eval_instructions, desc="Evaluating Instructions")):
            ...
            oss_image_folder_all = f'/data/oss_bucket_0/qihang/gui_agent/images/{task_name}/device0/instruction_{i+1+done_index}/sample_0'
            oss_text_folder_all = f'/data/oss_bucket_0/qihang/gui_agent/instructions/{task_name}/device0/instruction_{i+1+done_index}/sample_0'
